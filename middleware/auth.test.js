...
describe('ensureAdminOrCurrUser', function () {
  ...
  test('ensures current user', function () {
    expect.assertions(1);
    const req = { params: { username: 'user1' } };
    const res = { locals: { user: { username: 'user1', is_admin: false } } };
    const next = function (err) {
      expect(err).toBeFalsy();
    };
    ensureAdminOrCurrUser(req, res, next);
  ...
  test('unauthorized', function () {
    expect.assertions(1);
    const req = { params: { username: 'user1' } };
    const res = { locals: { user: { username: 'user2', is_admin: false } } };
    const next = function (err) {
      expect(err instanceof UnauthorizedError).toBeTruthy();
    };
    ensureAdminOrCurrUser(req, res, next);
